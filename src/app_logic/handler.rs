 ...
 * `StateManagerOperations` for tree state management. It also now includes
 * internal logic for estimating token counts of selected files.
...
impl MyAppLogic {
     ...
     * Sets up default application states, an empty command queue, and initializes
     * the token count to zero.
     */
    pub fn new(
        config_manager: Arc<dyn ConfigManagerOperations>,
        profile_manager: Arc<dyn ProfileManagerOperations>,
     ...
     * is active. Commands generated are enqueued. This also triggers an initial
     * token count calculation if a profile is loaded.
     */
     ...
     * It iterates through the `file_nodes_cache`, reads the content of each selected
     * file, and uses `estimate_tokens_simple_whitespace` to count its tokens.
     * The total count is stored in `self.current_token_count`. File read errors
     * are logged, but the process continues. The final count and statistics are
     * logged via `log::debug!`.
     */
    fn _recalculate_and_log_token_count(&mut self) {
                    ...
                    match fs::read_to_string(&node.path) {
                        Ok(content) => {
                            ...
                            *current_total_tokens += tokens_in_file;
                            // Optional: log tokens per file for very detailed debugging
                            // log::trace!("File: {:?}, Tokens: {}", node.path, tokens_in_file);
                        }
                        Err(e) => {
                            *files_failed += 1;
                            log::warn!(
        ...
        log::info!(
            "Token count updated: {} tokens from {} selected files ({} files failed to read).",
    ...
    fn handle_treeview_item_toggled(
        ...
        if let Some(path_for_model_update) = path_of_toggled_node {
            ...
            self._recalculate_and_log_token_count(); // New call
        } else {
            log::error!(
    ...
    /*
     * Activates a given profile: sets it as current, scans its root folder,
     * applies its selection state to the scanned files, refreshes the TreeView,
     * updates window title and archive status, and finally shows the window.
     * This method now also triggers a token count calculation after the profile
     * is fully processed and before the window is shown.
     */
...
impl MyAppLogic {
    ...
    // Getter for current_token_count for tests
