 ...
 * logic for estimating token counts of selected files and requesting
 * their display in the UI.
...
impl MyAppLogic {
     ...
     * logged via `log::debug!`. If a main window is present, a `PlatformCommand`
     * is enqueued to update the status bar with the token count.
     */
    ...
    fn handle_treeview_item_toggled(
        ...
        if let Some(path_for_model_update) = path_of_toggled_node {
            ...
            self._update_token_count_and_request_display(); // New call
        } else {
            log::error!(
